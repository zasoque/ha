...
/**
 * @swagger
 * /api/v1/maps/lands/{landId}/solidity:
 *   get:
 *     summary: Determine the solidity range of a specific land (costs money).
 *     description: This endpoint allows an authenticated user to investigate the solidity of a land by performing a "solidity survey" at a given level. This action costs money, which is deducted from the specified account.
 *     tags:
 *       - Maps - Lands
 *     security:
 *       - cookieAuth: []
 ...
 *           type: integer
 *           minimum: 1
 *         description: The level of solidity investigation. Higher levels provide more precise ranges but cost more.
 *       - in: query
 *         name: account_id
 *         required: true
 *         schema:
 ...
 *     responses:
 *       200:
 *         description: Returns the minimum and maximum possible solidity values for the land at the requested investigation level.
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                   type: number
 *                   format: float
 *                   description: The minimum possible solidity value.
 *                 max:
 *                   type: number
 *                   format: float
 *                   description: The maximum possible solidity value.
 *       400:
...
export const GET: RequestHandler = async ({ params, cookies, url }) => {
	...
	await query(
		...
		[account_id, -cost, 'withdrawal', `토지 #${landId} 강도 조사 ${level}단계 비용`]
	);

	const solidity = land[0].solidity;

	const max = Math.ceil(solidity * Math.pow(2, level)) / Math.pow(2, level);
	const min = Math.floor(solidity * Math.pow(2, level)) / Math.pow(2, level);
